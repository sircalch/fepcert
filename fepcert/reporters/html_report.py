"""
Interactive HTML report dashboard generator for FEPCert.
"""
...
<head>
    ...
    <title>FEPCert Alchemical Free Energy Quality Report</title>
    <style>
        :root {
            --bg-color: #0f172a;
            --card-bg: #1e293b;
    ...
    <div class="container">
        <header class="header">
            <div class="title-group">
                <h1>FEPCert Free Energy Validation Report</h1>
                <p>{{ report.metadata.transformation }} &bull; {{ report.free_energy.method }} ({{ report.metadata.engine }})</p>
            </div>
            <div>
        ...
        <div class="grid-cards">
            <div class="card">
                <div class="card-label">Calculated &Delta;G</div>
                <div class="card-value">{{ "%.2f"|format(report.free_energy.delta_g) }} &plusmn; {{ "%.2f"|format(report.free_energy.delta_g_error) }}</div>
                <div class="card-subtext">{{ report.free_energy.unit }} ({{ report.free_energy.method }})</div>
            </div>
            <div class="card">
                <div class="card-label">Min Adjacent Overlap</div>
                <div class="card-value">
                    {% if report.overlap_result %}
                    {{ "%.1f"|format(report.overlap_result.min_adjacent_overlap*100) }}%
                    {% else %}
                    N/A
                    {% endif %}
                </div>
                <div class="card-subtext">
                    {% if report.overlap_result %}
                    {{ report.overlap_result.status }} (Mean: {{ "%.1f"|format(report.overlap_result.mean_adjacent_overlap*100) }}%)
                    {% else %}
                    No overlap matrix
                    {% endif %}
                </div>
            ...
            <div class="card">
                <div class="card-label">Hysteresis & Dissipation</div>
                <div class="card-value">
                    {% if report.convergence_result %}
                    {{ "%.2f"|format(report.convergence_result.final_hysteresis) }} kcal/mol
                    {% else %}
                    N/A
                    {% endif %}
                </div>
                <div class="card-subtext">
                    {% if report.convergence_result %}
                    W_diss = {{ "%.2f"|format(report.convergence_result.dissipated_work) }} kcal/mol
                    {% else %}
                    Time series pending
                    {% endif %}
                </div>
            ...
            <div class="card">
                <div class="card-label">Cycle Closure Error</div>
                <div class="card-value">
                    {% if report.cycle_result %}
                    {{ "%.2f"|format(report.cycle_result.cycle_rmse) }} kcal/mol
                    {% else %}
                    PASS
                    {% endif %}
                </div>
                <div class="card-subtext">
                    {% if report.cycle_result %}
                    {{ report.cycle_result.n_cycles }} cycles evaluated
                    {% else %}
        ...
        <h2 class="section-title">Alchemical Simulation Quality Details</h2>
        <table>
            <thead>
                <tr>
            ...
            <tbody>
                <tr>
                    <td><strong>Statistical Precision</strong></td>
                    <td>Error &sigma; = {{ "%.2f"|format(report.free_energy.delta_g_error) }} {{ report.free_energy.unit }}</td>
                    <td>&sigma; &le; 0.30 kcal/mol</td>
                    <td><span class="tag tag-{{ report.free_energy.status.lower() }}">{{ report.free_energy.status }}</span></td>
                    <td style="color: var(--text-secondary)">{{ report.free_energy.diagnostic_message }}</td>
                </tr>
                {% if report.overlap_result %}
                <tr>
                    <td><strong>Phase Space Overlap (&Pi;)</strong></td>
                    <td>Min &Pi; = {{ "%.1f"|format(report.overlap_result.min_adjacent_overlap*100) }}%</td>
                    <td>Min &Pi; &ge; 3.0% (Klimovich)</td>
                    <td><span class="tag tag-{{ report.overlap_result.status.lower() }}">{{ report.overlap_result.status }}</span></td>
                    <td style="color: var(--text-secondary)">{{ report.overlap_result.diagnostic_message }}</td>
                </tr>
                {% endif %}
                {% if report.convergence_result %}
                <tr>
                    <td><strong>Time-Series Hysteresis</strong></td>
                    <td>&Delta;&Delta;G_hyst = {{ "%.2f"|format(report.convergence_result.final_hysteresis) }} kcal/mol</td>
                    <td>Hysteresis &le; 0.40 kcal/mol</td>
                    <td><span class="tag tag-{{ report.convergence_result.status.lower() }}">{{ report.convergence_result.status }}</span></td>
                    <td style="color: var(--text-secondary)">{{ report.convergence_result.diagnostic_message }}</td>
                </tr>
                {% endif %}
                {% if report.cycle_result %}
                <tr>
                    <td><strong>Thermodynamic Cycle Closure</strong></td>
                    <td>Cycle RMSE = {{ "%.2f"|format(report.cycle_result.cycle_rmse) }} kcal/mol</td>
                    <td>RMSE &le; 0.50 kcal/mol</td>
                    <td><span class="tag tag-{{ report.cycle_result.status.lower() }}">{{ report.cycle_result.status }}</span></td>
                    <td style="color: var(--text-secondary)">{{ report.cycle_result.diagnostic_message }}</td>
                </tr>
                {% endif %}
            </tbody>
        </table>
        ...
        <div class="box" style="border-left: 4px solid var(--warn-color);">
            <h3 style="color: var(--warn-color); margin-bottom: 0.5rem;">Diagnostic Recommendations & Sampling Alerts</h3>
...
def generate_fepcert_html_report(
